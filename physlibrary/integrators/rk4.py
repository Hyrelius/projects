...
def rk4_step(diffrential_equation, state, dt, t):
    ...
    k2 = diffrential_equation(state + k1 * (dt/2), t + dt/2 )
    k3 = diffrential_equation(state + k2 * (dt/2), t + dt/2 )
    k4 = diffrential_equation(state + k3 * dt, t + dt)
    
    return state + dt*(k1/6 + k2/3 + k3/3 + k4/6)
